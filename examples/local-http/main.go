...
func main() {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 4:
			_, _ = io.WriteString(w, `{"plan":"local","period_start":"2026-08-01","period_end":"2026-08-31","resets_at":"2026-09-01T00:00:00Z","included_credits":null,"used_credits":0,"remaining_credits":null}`)
		default:
	...
	token, _, err := c.CreateClientToken(ctx, january.CreateClientTokenRequest{EndUserID: "example-user", Scopes: []string{"foods:read"}})
	if err != nil {
		log.Fatal(err)
	}
	revoked, _, err := c.RevokeClientTokens(ctx, january.RevokeClientTokensRequest{EndUserID: "example-user"})
	if err != nil {
		log.Fatal(err)
	}
	credits, _, err := c.GetCredits(ctx)
	if err != nil {
		log.Fatal(err)
	}
	if requests.Load() != 4 || revoked == nil || revoked.RevokedCount != 500 {
		log.Fatal("expected four calls and one revoke")
	}
	fmt.Printf("Local HTTP consumer passed: %d foods, token expires in %ds, one revocation request revoked %d, plan %s, %d requests.\n", len(foods.Items), token.ExpiresIn, revoked.RevokedCount, credits.Plan, requests.Load())
}
